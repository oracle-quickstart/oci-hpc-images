...
variable "features" {
  type    = string
  default = "OCA-DOCA-OFED-3.1.0-AMD-ROCM-702"
}
...
variable "build_groups" {
  default = [ "kernel_parameters", "oci_hpc_packages", "mofed_doca_310", "hpcx_2212", "openmpi_508", "amd_rocm_702", "oca_153_ubuntu"]
}
...
build {
  ...
  provisioner "ansible" {
    ...
    extra_arguments = var.OpenSSH9 ? [ "-e", local.ansible_args, "--scp-extra-args", "'-O'", "-vvv"] : [ "-vvv", "-e", local.ansible_args]
